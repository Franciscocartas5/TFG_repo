	...
	g[h]={
		...
		settings:{
			...
			hasFlash:function(){
				...
				else try{new ActiveXObject("ShockwaveFlash.ShockwaveFlash");
					return true
				}
				catch(a){
			...
			flashError:function(){
				...
				if(this.wav)
					c+=' <a href="'+this.wav+'">Download audio file</a>.';
			...
			loadError:function(){
				...
				g[h].helpers.removeClass(this.wrapper,b.loadingClass);
				g[h].helpers.addClass(this.wrapper,b.errorClass);
				a.innerHTML='Error loading: "'+this.wav+'"'
			},
			init:function(){
			...
			updatePlayhead:function(b){
				...
				document.getElementById("contenido").innerHTML = String(b)+':'+String(c)

		        var pt = document.getElementById('contenido-archivo-p').innerText;
	            var p = pt.split(',').map(function(item) {
	             return parseInt(item, 10);
	            });;

		        var qt = document.getElementById('contenido-archivo-q').innerText;
	            var q = qt.split(',').map(function(item) {
	             return parseInt(item, 10);
	            });;
		       
		       				
		        t_interpret = b*60+c;
		        frame = t_interpret*(44100/512.0)
  	            index = q.indexOf(Math.round(frame))

  	            var t_partiture = p[index]*512/44100;
				
				var tiempo = t_partiture*1000 //en ms

				contenido_mpos = document.getElementById("contenido_mpos").value
			
				for(i=0,f = i+1;i<f;i++){

					if(tiempo >= contenido_mpos.getElementsByTagName('events')[0].children[i].attributes[1].value){
						if(tiempo < contenido_mpos.getElementsByTagName('events')[0].children[i+1].attributes[1].value){
							var compas = contenido_mpos.getElementsByTagName('events')[0].children[i].attributes[0].value;
							//console.log(compas)
						}
						else{
							f = f+1;
				...
				osc.open()								
					...
					function promiseSqrt(compas){
					    return new Promise(function (fulfill, reject){
					        console.log('START execution with value =', compas);
					        setTimeout(function() {
					            fulfill({ value: parseInt(compas,10)+1 });
					        }, 0 | Math.random() * 50);
					    });
					}

					var p = [];
					    p.push(promiseSqrt(compas));

					Promise.all(p).then(function(results) {
					    results.forEach(function(obj) {
					        console.log('END execution with value =', obj.value)
						  var message = new OSC.Message('/select-measure', obj.value);
						  osc.send(message);					        
					    })

					});

				// document.getElementById('contenido').addEventListener('change', () => {
				//   var message = new OSC.Message('/select-measure', parseInt(compas , 10));
				//   osc.send(message);
				// });
		...
		newInstance:function(b,a){
			...
			if(b.getAttribute("loop")!=null)
				c.loop=true;
			if(b.getAttribute("preload")=="auto")
				c.preload=true;
		...
		createPlayer:function(b,a,c){
			...
			if(e.outerHTML&&!document.createElement("audio").canPlayType){
				...
				d.innerHTML=a.markup;d.appendChild(e);
		...
		attachFlashEvents:function(b,a){
			...
			a.loadStarted=function(){
				a.swfReady=true;
				a.settings.preload&&a.element.init(a.wav);
		...
		helpers:{
			...
			injectCss:function(b,a){
				...
				i=d.firstChild;f=document.createElement("style");
				if(d){
		...
		events:{
			...
			ready:function(){
				return function(b){
					var a=window, c=false, d=true, e=a.document, i=e.documentElement, f=e.addEventListener?"addEventListener":"attachEvent", k=e.addEventListener?"removeEventListener":"detachEvent", n=e.addEventListener?"":"on"
					m=function(l){
